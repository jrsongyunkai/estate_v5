  ...
  export default {
    ...
    data() {
      return {
        ...
        options: { nameY: this.$t('unitPiece'), xAxis: [], series: [], title: { show: true, text: this.$t('tfthreePhaseAlarmSituation') } },
        params: { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: '17,18,19', date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' },
      }
    },
    methods: {
      search(e) {
        ...
        this.$nextTick(() => {
          warnStates(this.params).then(res => {
            ...
            this.options.series.push({
              name: this.$t('threePhaseAlarmNumber'),
              type: 'line',
              data: [],
              smooth: true,
              areaStyle: {
            ...
            if (this.params.dateType === 2) {
              this.options.title.text = this.$t('thisMonthThreePhaseAlarmSituation')
            } else {
              this.options.title.text = this.$t('tfthreePhaseAlarmSituation')
            }
      ...
      reset() {
        this.params = { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: '17,18,19', date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' }
        this.init()
      },
    },
    created() {
