<template>
  <div>
    <warnS v-show="!showDetail" @lookDeviceDetails="lookDeviceDetails" :datas="datas" @sendPage="changePage" @sendSize="changesize" @search="search" @reset="reset" @sendDate="changeDM">
  ...
  export default {
    ...
    data() {
      return {
        ...
        options: { nameY: this.$t('unitPiece'), xAxis: [], series: [], title: { show: true, text: this.$t('allDayArcAlarmSituation') } },
        params: { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: 11, date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' },
      }
    },
    methods: {
      search(e) {
        ...
        this.$nextTick(() => {
          warnStates(this.params).then(res => {
            ...
            if (this.params.dateType === 2) {
              this.options.title.text = this.$t('thisMonthArcAlarmSituation')
            } else this.options.title.text = this.$t('allDayArcAlarmSituation')
      ...
      reset() {
        this.params = { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: 11, date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' }
        this.init()
      },
    },
    created() {
