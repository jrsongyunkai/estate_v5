...
<script>
  ...
  export default {
    name: 'safetyMonitor-shortCircuitWarn',
    data() {
      return {
        datas: {
          data1: [],
        ...
        options: {
          xAxis: [],
          nameY: this.$t('unitPiece'),
          series: [],
          title: { show: true, text: this.$t('allDayShortCircuitAlarm'), textStyle: { color: '#000', fontSize: 16 } },
        },
        params: { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: '1', date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' },
      }
    },
    ...
    methods: {
        ...
        this.$nextTick(() => {
          warnStates(this.params).then(res => {
            ...
            this.options.series.push({
              name: this.$t('numberOfShortCircuitAlarm'),
              type: 'line',
              data: [],
              smooth: true,
              areaStyle: {
            ...
            if (this.params.dateType === 2) {
              this.options.title.text = this.$t('thisMonthShortCircuitAlarm')
            } else {
              this.options.title.text = this.$t('allDayShortCircuitAlarm')
            }
      ...
      reset() {
        this.params = { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: '1', date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' }
        this.getEchartsData()
        this.getTableData()
      },
    },
