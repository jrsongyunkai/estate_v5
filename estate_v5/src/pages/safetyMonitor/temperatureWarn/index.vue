...
<script>
  ...
  const symbolImg = 'image://' + require('@/assets/images/max.png')
  const symbolImg2 = 'image://' + require('@/assets/images/min.png')
  export default {
    name: 'safetyMonitor-temperatureWarn',
      ...
      return {
        datas: {
          ...
          tableColumns1: [
            ...
            { title: this.$t('phaseLineOrTerminal'), slot: 'phase' },
            { title: this.$t('alarmTime'), key: 'time' },
          ],
          total: 0,
        },
        options: { nameY: this.$t('unitPiece'), xAxis: [], series: [], title: { show: true, text: this.$t('temperatureAlarmConditionFor24hours') } },
        params: { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: 7, date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' },
      }
    },
    methods: {
      search(e) {
        ...
        this.$nextTick(() => {
          warnStates(this.params).then(res => {
            ...
            this.options.series.push({
              name: this.$t('temperatureAlarmNumber'),
              type: 'line',
              data: [],
              smooth: true,
              markPoint: {
                data: [
                  { type: 'max', symbol: symbolImg, symbolOffset: [0, -30], label: { fontSize: 11, align: 'center', padding: [0, 5, 33, 5], color: '#ff4f4f' } },
                  { type: 'min', symbolOffset: [0, -30], symbol: symbolImg2, label: { color: '#00ba8a', fontSize: 11, align: 'center', padding: [0, 5, 33, 5] } },
                ],
            ...
            if (this.params.dateType === 2) {
              this.options.title.text = this.$t('currentMonthTemperatureAlarmCondition')
            } else {
              this.options.title.text = this.$t('temperatureAlarmConditionFor24hours')
            }
      ...
      reset() {
        this.params = { page: 1, rows: 10, projectCode: this.$store.state.projectCode, mac: '', dateType: 1, typeNumbers: 7, date: this.$Date().format('YYYY-MM-DD'), nodeIds: '' }
        this.init()
      },
    },
    created() {
