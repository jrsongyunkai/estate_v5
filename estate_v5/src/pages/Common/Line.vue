<template>
  <div>
    ...
    <div ref="eChart" class="line" :style="baseStyle"></div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        let that = this
        const options = {
          title: { show: this.options.title.show, text: this.options.title.text, textStyle: { color: '#FFF', fontSize: 16 }, left: 20, top: 10 },
          grid: { x: 50, y: 65, x2: 60, y2: this.options.grid ? this.options.grid : 80, borderWidth: 0, bottom: '50' },
          tooltip: {
            trigger: 'axis',
            confine: true,
