    ...
    <div v-else id="alarms">
        ...
        <Col style="width: 100%">
          <Row type="flex" justify="space-between">
            <Breadcrumb separator-class="el-icon-arrow-right">
              <BreadcrumbItem class="pointer" @click.native="$router.push(pathUrl)" style="color: #7b7b7b">{{ $t('back') }}</BreadcrumbItem>
              <BreadcrumbItem>{{ $t('alramWarnList') }}</BreadcrumbItem>
            </Breadcrumb>
            <Col class="history-text pointer">
              <Button type="text" @click="$router.push({ path: '/AlarmHistory' })">{{ $t('historyData') }}{{ '>>' }}</Button>
            </Col>
          </Row>
          <!-- <Row class="title">
            <Col>
                <i class="icon-v5 icon-v5-huifu"></i>
                {{$t('message.countedOnlyonce')}}
            </Col>
        </Row> -->
          <Row class="alarms_typeList">
            <Col :span="24">
              <Form :inline="true" :model="quantityForm" class="demo-form-inline" @submit.native.prevent>
                <FormItem class="fl">
                  <span class="ml-20 fs-18" v-if="alarmData.currentMonthAlarmNum">{{ $t('numberOfAlarmsInThisMonth') + '：' + alarmData.currentMonthAlarmNum }}</span>
                  <span class="ml-20 fs-18" v-if="alarmData.lastMonthAlarmNum">{{ $t('numberOfAlarmsInLastMonth') + '：' + alarmData.lastMonthAlarmNum }}</span>
                  <span class="ml-20 fs-18" v-if="alarmData.totalAlarmNum">{{ $t('cumulativeNumberOfAlarms') + '：' + alarmData.totalAlarmNum }}</span>
                </FormItem>
                <FormItem>
                ...
                <FormItem>
                  ...
                  <span class="block">
                    <DatePicker :clearable="false" :disabled="jobFlag" v-model="quantityForm.month" value-format="yyyy-MM-dd" type="daterange" :editable="false" :separator="$t('to')" @on-change="handleChangeDate"></DatePicker>
                  </span>
                </FormItem>
                <FormItem>
                ...
                <FormItem>
                  <Select :clearable="true" v-model="quantityForm.addr" :disabled="jobFlag" v-width="140" :placeholder="$t('lineMacCode')">
                    <Option v-for="it in addrOptions" :key="it.value" :value="it.value">{{ it.label }}</Option>
                  </Select>
                </FormItem>
                ...
                <FormItem style="width: 250px">
                  <Select :max-tag-count="2" :clearable="true" v-model="quantityForm.type" :disabled="jobFlag" @on-change="handleChange" @visible-change="handleFocus" multiple multiple-limit="3" :placeholder="$t('alarmType')">
                    <Option v-for="item in alarmTypes" :key="item.typeNumber" :label="item.info" :value="item.typeNumber" :disabled="item.disabled"></Option>
                  </Select>
                </FormItem>
                <FormItem>
                  <Select :clearable="true" v-model="quantityForm.oqp1" :disabled="jobFlag" multiple multiple-limit="3" style="width: 255px" :placeholder="$t('state')">
      ...
      <div style="padding: 0 30px">
        <Table :data="quantityData" :columns="columns" @on-selection-change="handleSelectionChange">
          ...
          <template slot-scope="{ row }" slot="othersMessage">
            <Poptip placement="top" trigger="click">
              <div slot="content">
                <div v-if="row.valueDescribe.from">
                  <div v-if="row.valueDescribe.from === 'slaveDevice'">{{ $t('fromDeviceId', { id: row.valueDescribe.slaveDeviceId, value: row.valueDescribe.value }) }}</div>
                  <div v-if="row.valueDescribe.from === 'loop'">{{ $t('loop') + '：' + row.valueDescribe.loopName }}</div>
                </div>
              </div>
              <i class="icon-v5 icon-v5-nenghaofenxi" v-show="row.valueDescribe && Object.keys(row.valueDescribe).length > 0"></i>
            </Poptip>
            {{ row.others || '' }}
          </template>
          <template slot-scope="{ row }" slot="value">
            <Poptip placement="top" trigger="click" v-if="row.equipmentType !== 23 && row.equipmentType !== 24">
              <div slot="content">
                {{ alarmInfo }}
              </div>
              <i
                class="icon-v5 icon-v5-nenghaofenxi"
                v-show="(row.valueDescribe !== '' && JSON.stringify(row.valueDescribe) !== '{}' && row.valueDescribe.elecType && row.typeNumber !== 2021) || (row.typeNumber === 2021 && row.valueDescribe.from && row.valueDescribe.from !== 'loop')"
                @click="handleValue(row.valueDescribe)"></i>
            </Poptip>
          </template>
          ...
          <template slot-scope="{ row }" slot="isSupplementary">
            <div>{{ row.valueDescribe.his ? $t('yes') : $t('no') }}</div>
          </template>
        </Table>
      </div>
...
<script>
  ...
  import {
    ...
    alarmUrgePush,
    queryAlarmForConfig,
    exportAlarmListOfPage,
    statisticalOfBoxAlarmClassified,
  ...
  export default {
    data() {
      return {
        ...
        columns: [
          ...
          { title: this.$t('otherMessage'), slot: 'othersMessage', align: 'center' },
          { title: this.$t('alarmTime'), key: 'time', align: 'center', sortable: true },
          { title: this.$t('serverReceivingTime'), key: 'create_time', align: 'center', sortable: true },
          { title: this.$t('isSupplementary'), slot: 'isSupplementary', align: 'center' },
    ...
    mounted() {
      if (this.$route.params.projectCode) {
        this.checkedKeys = this.$route.params.projectCode.split(',')
        this.project = this.checkedKeys
      } else if (this.$store.state.projectCode) {
        this.project = [this.$store.state.projectCode]
      } else {
        this.checkedKeys = this.$store.state.allProjects.data.map(it => it.projectCode)
      }
      this.init()
      if (!this.pathUrl) {
        this.pathUrl = 'visualScreen'
      }
      this.queryConfig()
    },
    beforeRouteEnter(to, from, next) {
      next(vm => {
        if (from.fullPath === '/visualScreen') {
          store.commit('alarmRouter', '/visualScreen')
        } else if (from.fullPath === '/console/project') {
          store.commit('alarmRouter', '/console/project')
        }
        if (from.fullPath === '/') {
          vm.pathUrl = 'visualScreen'
        } else if (from.fullPath === '/Alarm') {
          vm.pathUrl = 'visualScreen'
        } else if (from.fullPath === '/AlarmHistory') {
          if (store.state.alarmRouter) {
            vm.pathUrl = store.state.alarmRouter
          } else {
            vm.pathUrl = 'visualScreen'
          }
        } else {
      ...
      // this.quantityForm.type = this.$route.params.typeNumber ? this.$route.params.typeNumber : ''
    },
    methods: {
      init() {
      ...
      async queryConfig() {
        let res = await queryAlarmForConfig({})
        if (res.success) {
      ...
      async queryAlarm() {
        let params = {
          ...
          projectCode: this.project.toString(),
          ...
          oqp1: Array.isArray(this.quantityForm.oqp1) ? this.quantityForm.oqp1.join(',') : this.quantityForm.oqp1,
          startTime: dayjs(this.quantityForm.month[0]).format('YYYY-MM-DD'),
          endTime: dayjs(this.quantityForm.month[1]).format('YYYY-MM-DD'),
        }
        if (this.$func.checkDateThreeMonths(params.startTime, params.endTime) === 'false') {
          this.$Message.error({ content: this.$t('limitThreeMonths') })
          return false
        }
        let res = await queryAlarmListOfPageById(params)
        if (res.success) {
      ...
      handleValue(obj) {
        ...
        if (obj.elecType === 9) {
          ...
          this.alarmInfo = `${type}: ${obj.value ? obj.value : ''}${obj.unit ? obj.unit : ''}`
        } else if (obj.elecType === 12) {
          this.alarmInfo = `${type}: ${obj.value + 'Lux'} ${'(' + this.$t('threshold') + ':' + obj.threshold + ')'}`
        } else if (obj.elecType === 14) {
          this.alarmInfo = `${type}: ${obj.value1 + obj.unit1}`
        } else if (obj.elecType === 25) {
          this.alarmInfo = `${type}: ${obj.value + 'Lux'} ${'(' + obj.threshold + ')'}`
        } else {
          this.alarmInfo = `${type}: ${obj.value}${obj.unit} ${'(' + this.$t('threshold') + ':' + obj.threshold + ')'}`
        }
      },
      handleReset() {
        this.quantityForm.month = [dayjs().startOf('month').format('YYYY-MM-DD'), dayjs().format('YYYY-MM-DD')]
        this.quantityForm.mac = ''
        this.quantityForm.addr = ''
        this.quantityForm.jobNumber = ''
        this.quantityForm.type = ['-100']
      ...
      async handleCommand(command) {
        ...
        if (command === 'statistics') {
          let params = {
            ...
            processStatus: this.quantityForm.oqp1.length > 0 ? this.quantityForm.oqp1.join(',') : '',
          }
          if (this.quantityForm.addr) params.addr = this.quantityForm.addr
          const res = await statisticalOfBoxAlarmClassified(qs.stringify(params))
          this.$func.getPostResFile(res, '项目设备告警分类统计表')
        } else {
          let provinceId = this.$store.state.provinceId ? this.$store.state.provinceId : ''
          let cityId = this.$store.state.cityId ? this.$store.state.cityId : ''
          let countyId = this.$store.state.countyId ? this.$store.state.countyId : ''
          let oqp1 = this.quantityForm.oqp1 ? this.quantityForm.oqp1.join(',') : ''
          ...
          if (this.quantityForm.addr) params.addr = this.quantityForm.addr
          const res = await exportAlarmListOfPage(qs.stringify(params))
          this.$func.getPostResFile(res, '报警预警列表详细')
        }
      },
