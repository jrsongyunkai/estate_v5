<template>
  <div class="table-tags" ref="cellTags">
    <div class="tags-box" ref="tagsBox">
      <Button v-if="isEdit" type="primary" size="small" ghost @click.stop="handleAddList" class="choice">{{ $t('add') }}</Button>
      <Tag v-for="(tag, index) in tagList" :key="index" :closable="isEdit" :name="tag.label" :disable-transitions="false">
        <span>{{ tag.label }}</span>
      </Tag>
    </div>
    <div class="tags-box-list">
      <Button type="primary" v-if="isEdit" size="small" ghost @click.stop="handleAddList" class="choice">{{ $t('add') }}</Button>
      <template v-for="(tag, idx) in tagList">
        <Tag :key="idx" :closable="isEdit" :name="tag.label" v-if="idx < overIndex || !showPip" :disable-transitions="false" @on-close="handleTypesClose(idx)">
          <span>{{ tag.label }}</span>
        </Tag>
      </template>
    </div>
    <Tooltip v-if="showPip && moreLength > 0" transfer transfer-class-name="table-tag-tips">
      ...
      <template #content>
        <div class="title-head-tips">{{ $t('allSelected') }}（{{ sumLength }}）</div>
        <div class="content-tag">
          <Tag v-for="(tag, idx) in tagList" :key="tag.projectCode" :closable="isEdit" :name="tag.label" :disable-transitions="false" @on-close="handleTypesClose(idx)">
            <span>{{ tag.label }}</span>
          </Tag>
        </div>
      </template>
