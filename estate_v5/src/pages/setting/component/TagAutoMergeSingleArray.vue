<template>
  <div class="table-tags" ref="cellTags">
    <div class="tags-box" ref="tagsBox">
      ...
      <Tag v-for="tag in typeNumbers" :key="tag.projectCode" :closable="isEdit" :name="tag.projectName" :disable-transitions="false">
        <span>{{ tag.projectName }}</span>
      </Tag>
    </div>
    <div class="tags-box-list">
      ...
      <template v-for="(tag, idx) in typeNumbers">
        <Tag :key="tag.projectCode" :closable="isEdit" :name="tag.projectName" v-if="idx < overIndex || !showPip" :disable-transitions="false" @on-close="handleTypesClose(idx)">
          <span>{{ tag.projectName }}</span>
        </Tag>
      </template>
    </div>
    <Tooltip v-if="showPip && moreLength > 0" transfer transfer-class-name="table-tag-tips">
      ...
      <template #content>
        ...
        <div class="content-tag">
          <Tag v-for="(tag, idx) in typeNumbers" :key="tag.projectCode" :closable="isEdit" :name="tag.projectName" :disable-transitions="false" @on-close="handleTypesClose(idx)">
            <span>{{ tag.projectName }}</span>
          </Tag>
        </div>
      </template>
...
<script>
  export default {
    ...
    computed: {
      moreLength() {
        return this.typeNumbers.length - this.overIndex
      },
      sumLength() {
        return this.typeNumbers.length
      },
    },
    methods: {
      handleAddAlarm() {
