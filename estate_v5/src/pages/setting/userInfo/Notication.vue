    ...
    <Col :span="24">
      <Table :data="data" height="480" width="100%" :columns="columnTable">
        ...
        <template #url="{ row, index }">
          <Input size="small" v-model="row.url" @on-change="onAddOptionChange(row.url, 'url', index)" :disabled="!row.isEdit"></Input>
        </template>
        <template #salt="{ row, index }">
          <Input size="small" v-model="row.salt" @on-change="onAddOptionChange(row.salt, 'salt', index)" :disabled="!row.isEdit"></Input>
        </template>
    ...
    data() {
      return {
        ...
        columnTable: [
          ...
          { title: this.$t('interfaceUrl'), slot: 'url', width: 140 },
          { title: this.$t('signatureSaltValue'), slot: 'salt', width: 140 },
    ...
    methods: {
      ...
      handleSave(row) {
        ...
        let params = {
          ...
          channelParams: row.url + ',' + row.salt + (row.silenceTimeout ? '^' + row.silenceTimeout : ''),
        }
