<template>
  <div :class="wrapperClasses" v-click-outside:[capture].mousedown="handleClose" v-click-outside:[capture].touchstart="handleClose" v-click-outside:[capture]="handleClose">
    ...
    <transition name="transition-drop">
      <Drop @click.native="handleTransferClick" v-show="opened" :class="dropdownCls" :placement="placement" ref="drop" :eventsEnabled="eventsEnabled" :data-transfer="transfer" :transfer="transfer" v-transfer-dom>
        <div>
          <component
            :is="panel"
            ref="pickerPanel"
...
<script>
  ...
  export default {
    ...
    components: { Drop, RangeTimePickerPanel },
    ...
    computed: {
      ...
      visualValue() {
        const value = this.formatDate(this.internalValue)
        const addOneMinutesValue = this.addOneMinutes(value)
        return addOneMinutesValue
      },
      isConfirm() {
    ...
    methods: {
      ...
      addOneMinutes(val) {
        const splitArr = cloneDeep(val)
          .split('-')
          .map(item => item.trim())
        if (!dayjs('2024-03-28 ' + splitArr[1]).isValid()) return ''
        let splitIndex1 = dayjs('2024-03-28 ' + splitArr[1])
          .add(1, 'minute')
          .format('HH:mm')
        if (splitArr[1] === '23:59') {
          splitIndex1 = '24:00'
        }
        return splitArr[0] + ' - ' + splitIndex1
      },
