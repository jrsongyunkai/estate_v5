<template>
  <div>
    <Modal v-model="isShowModel" title="退费情况" class-name="tripping-modal" width="500" @on-cancel="$emit('close')">
      <Steps :current="situationCurrent" :status="situationStatus">
        <template v-if="refundNum">
          <Step title="申请退款" :content="refundNum.refundCreateTime"></Step>
          <template v-if="refundNum.status === 1">
            <Step title="退款处理中" :content="refundNum.refundCompletedTime"></Step>
          </template>
          <!-- 0-待退款，1-退款中，2-退款成功，3-退款失败，4-退款取消 -->
          <template v-if="refundNum.status !== 1">
            <Step :title="refundNum.status === 2 ? '退款成功' : '退款异常'" :content="refundNum.refundCompletedTime"></Step>
          </template>
        </template>
      </Steps>
      <div slot="footer"></div>
