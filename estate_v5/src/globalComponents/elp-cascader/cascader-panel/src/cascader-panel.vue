<template>
  <div :class="['elp-cascader-panel', border && 'is-bordered']" v-if="visible" @keydown="handleKeyDown">
    <cascader-menu ref="menu" v-for="(menu, index) in menus" :key="index" :nodes="menu" :index="index" :empty-text="emptyText" />
  </div>
</template>
...
<script>
  ...
  import { isEqual, isEmpty, valueEquals, coerceTruthyValueToArray } from 'element-ui/src/utils/util'

  import Store from './store'
  import { DefaultProps } from './constant'
  import CascaderMenu from './cascader-menu'
  import { focusNode, checkNode, getSibling, getMenuIndex } from './utils'
  import '../../theme/cascader-panel.less'
      ...
      scrollIntoView() {
        ...
        menus.forEach(menu => {
          ...
          if (menuElement) {
            ...
            const activeNode = menuElement.querySelector('.elp-cascader-node.is-active') || menuElement.querySelector('.elp-cascader-node.in-active-path')
