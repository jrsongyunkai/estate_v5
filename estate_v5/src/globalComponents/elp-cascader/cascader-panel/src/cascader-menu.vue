    ...
    <div class="elp-cascader-menu__wrap elp-cascader-menu__list">
      <div class="elp-search-check">
        <div v-if="labelAndCheckAllVisible" class="elp-search-check__check">
          ...
          <el-checkbox v-if="checkAllVisible" :value="menuCheckState.checked" :indeterminate="menuCheckState.indeterminate" @change="onMenuCheck">{{ $t('selectAll') }}</el-checkbox>
        </div>
        <div v-if="searchVisible" style="padding: 5px 20px">
          <el-input clearable v-model.trim="keyWordsTemp" :placeholder="menuPlaceholder" size="small" suffix-icon="el-icon-search" @input="handleSearchInput" />
        </div>
      </div>
      <div v-if="searchLoading" class="elp-cascader-menu__empty-text">{{ $t('loading') }}</div>
      <div v-else-if="isEmpty" class="elp-cascader-menu__empty-text">
        {{ emptyText }}
      </div>
      <recycle-scroller v-else v-slot="{ item, index }" :buffer="100" :items="filterNodes" :item-size="34" key-field="value" :style="{ height: scrollHeight }">
        <cascader-node :key="item.uid" :node="item" :node-id="`${menuId}-${index}`" :insertMode="searching" :aria-haspopup="item.hasChildren" :aria-owns="item.hasChildren ? menuId : null" @expand="isHover && handleExpand" />
      </recycle-scroller>
  ...
  export default {
    ...
    computed: {
      ...
      menuPlaceholder() {
        ...
        return _placeholders[this.index] || this.$t('enterContent')
      },
      checkAllVisible() {
