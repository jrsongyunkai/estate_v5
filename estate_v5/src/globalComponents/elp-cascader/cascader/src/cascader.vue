<template>
  <div
    ref="reference"
    :class="['elp-cascader', { 'is-disabled': isDisabled }, realSize && `elp-cascader--${realSize}`]"
    ...
    <!--  eslint-disable  -->
    <el-input ref="input" v-model="multiple ? presentText : inputValue" :size="realSize" :readonly="readonly" :disabled="isDisabled" :validate-event="false" :placeholder="placeholder" :class="{ 'is-focus': dropDownVisible }" @focus="handleFocus" @blur="handleBlur" @input="handleInput">
      <template slot="suffix">
        <i v-if="clearBtnVisible" key="clear" class="el-input__icon el-icon-circle-close" @click.stop="handleClear" />
        <i v-else key="arrow-down" :class="['el-input__icon', 'el-icon-arrow-down', dropDownVisible && 'is-reverse']" @click.stop="toggleDropDownVisible()" />
      </template>
    </el-input>

    <div v-if="multiple" class="elp-cascader__tags">
      <el-tag v-for="(tag, index) in presentTags" :key="tag.key" type="info" :size="tagSize" :hit="tag.hitState" :closable="tag.closable" disable-transitions @close="deleteTag(index)">
        <span>{{ tag.text }}</span>
      </el-tag>
      <input v-if="filterable && !isDisabled" v-model.trim="inputValue" type="text" class="elp-cascader__search-input" :placeholder="presentTags.length ? '' : placeholder" @input="e => handleInput(inputValue, e)" @click.stop="toggleDropDownVisible(true)" @keydown.delete="handleDelete" />
    </div>

    <transition name="el-zoom-in-top" @after-leave="handleDropdownLeave">
      <div v-show="dropDownVisible" ref="popper" :class="['elp-cascader__dropdown', 'el-popper', popperClass]">
        <elp-cascader-panel
          ref="panel"
          v-show="!filtering"
        ...
        <scrollbar ref="suggestionPanel" v-if="filterable" v-show="filtering" tag="ul" class="elp-cascader__suggestion-panel" view-class="elp-cascader__suggestion-list" @keydown.native="handleSuggestionKeyDown">
          <template v-if="suggestions.length">
            <li v-for="(item, index) in suggestions" :key="item.uid" :tabindex="-1" :class="['elp-cascader__suggestion-item', item.checked && 'is-checked']" @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
...
<script>
  ...
  import '../../theme/cascader.less'
  import ElpCascaderPanel from '../../cascader-panel'
